...
| :--- | :--- | :--- |
| **Declaration Syntax** | Use the `record` keyword followed by the components list (the record descriptor). | ```java public record Person(String name, int age) {} ``` |
...
| **Canonical Constructor** | A public constructor is generated that accepts all components and initializes the corresponding fields. This is the only constructor guaranteed to exist. | ```java Person p = new Person("Alice", 30); ``` |
| **Accessors (Getters)** | Public accessor methods are generated, named identically to the component fields (e.g., `name()` instead of `getName()`). | ```java String personName = p.name(); int personAge = p.age(); ``` |
| **Standard Methods** | Implements robust versions of `equals()`, `hashCode()`, and `toString()`, derived from all component fields. | ```java  Prints: Person[name=Alice, age=30] System.out.println(p); ``` |
...
| :--- | :--- | :--- |
| **Compact Canonical Constructor** | Used for argument validation or normalization. It omits the parameter list and explicit field assignments (`this.name = name`). The assignments are automatically performed at the end of the constructor block. | ```java public record PriceRange(double high, double low) { public PriceRange { if (high < low) { throw new IllegalArgumentException("High cannot be less than low"); } } } ``` |
| **Custom (Non-Canonical) Constructors** | Any constructor with a different signature must explicitly call the canonical constructor using `this()` as the first statement. | ```java public record Person(String name, String address) { public Person(String name) { this(name, "Unknown"); // Must call canonical constructor } } ``` |
...
| :--- | :--- | :--- |
| **Instance Methods** | You can define custom instance methods for utility or calculated values. | ```java public record Point(int x, int y) { public double getMagnitude() { return Math.sqrt(x*x + y*y); } } ``` |
| **Static Variables and Methods** | Records support static variables (often final constants) and static methods. | ```java public record Person(String name, String address) { public static String UNKNOWN_ADDRESS = "Unknown"; public static Person createUnnamed(String address) { return new Person("Unnamed", address); } } ``` |
...
| :--- | :--- | :--- |
| **Type Pattern in `if`** | Checks the type and, if matched, assigns the object to a binding variable of that specific type, eliminating the need for explicit casting. | ```java Object obj = "Hello"; if (obj instanceof String s) { System.out.println(s.length()); // s is automatically String type } ``` |
| **Scope of Pattern Variables** | The pattern variable (`s` in the example above) is only in scope where the pattern is guaranteed to have matched (i.e., inside the `if` block, or after a conditional check like `&&`). | ```java if (!(obj instanceof String s)) { return; } // s is in scope here ``` |
...
| :--- | :--- | :--- |
| **Type Pattern in `switch`** | Matches the type of the switch selector expression. | ```java static double convertToDouble(Object o) { return switch (o) { case Integer i -> i.doubleValue(); case String s -> Double.parseDouble(s); default -> 0d; }; } ``` |
| **Record Pattern (Destructuring)** | Matches a record and automatically deconstructs it, extracting components into new binding variables. Matching is done by position and compatible type. | ```java public record Point(int x, int y) {} public void printPoint(Object obj) { switch (obj) { case Point(int x, int y) -> System.out.println("x:" + x + ", y:" + y); default -> {} } } ``` |
| **Nested Record Patterns** | Allows deconstructing records that contain other records, enabling deep extraction of data. | ```java public record Line(Point start, Point end) {} public void checkLine(Object shape) { if (shape instanceof Line(Point(int x1, int y1), Point(int x2, int y2))) { System.out.println("Start at " + x1 + "," + y1); } } ``` |
| **Using `var`** | Type inference (`var`) can be used for binding variables in patterns. | ```java if (obj instanceof Point(var x, var y)) { // x and y are inferred as int } ``` |

#### 2. Advanced Switch Concepts

| Concept | Detail | Example Code |
| :--- | :--- | :--- |
| **Guarded Patterns** | Uses the `when` clause to apply an additional boolean condition after the initial pattern match. | ```java case String s when s.length() > 5 -> // Only execute if string length > 5 ``` |
| **Null Handling** | `switch` statements/expressions can explicitly handle null values using a dedicated `case null` label, preventing `NullPointerException`s. | ```java case null -> System.out.println("Input is null"); ``` |
...
| **Sealed Types & Exhaustiveness** | If the selector type is a **Sealed Interface** (which permits a known, finite set of subtypes), the compiler ensures all permitted subtypes are covered in the `case` labels, eliminating the need for a `default` case. | ```java sealed interface Shape permits Circle, Square {} // switch (shape) needs only case Circle c and case Square s ``` |
...
| **Unnamed Patterns/Variables** | The single underscore (`_`) marks a variable or pattern component that the developer does not care about, increasing readability when extraction is unnecessary. | *(If `Point` has components `(x, y)` but we only need `x`):* ```java case Point(int x, _) -> System.out.println("Only need x: " + x); ``` |

### C. Other Pattern Matching Contexts

| Context | Detail | Example Code |
| :--- | :--- | :--- |
| **Enhanced `for` loop** | Pattern matching can be used in enhanced `for` loops (preview feature) to deconstruct records during iteration. | ```java for (Point(int x, int y) p : listOfPoints) { System.out.println(x + y); } ``` |
