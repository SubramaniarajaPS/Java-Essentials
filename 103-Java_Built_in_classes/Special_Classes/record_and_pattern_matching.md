...
| **Null Handling** | `switch` statements/expressions can explicitly handle null values using a dedicated `case null` label, preventing `NullPointerException`s. | ``` case null -> System.out.println("Input is null"); ``` |
